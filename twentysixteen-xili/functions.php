...
function twentysixteen_xilidev_setup() {
	...
	if ( isset( $_GET['activated'] ) || ( ! isset( $_GET['activated'] ) && ( ! $xl_required_version || ! $class_ok ) ) ) {
		//REPLACE - php 7.2 add_action( 'admin_notices', create_function( '', 'echo "' . addcslashes( $msg, '"' ) . '";' ) );
		add_action(
			'admin_notices',
			function() use ( &$msg ) {
				echo $msg;
...
function twentysixteen_xili_credits() {
	...
	printf(
		/* translators: added in child functions by xili */
		esc_html__( 'Multilingual child theme of Twenty Sixteen by %1$s and %2$s', 'twentysixteen' ),
